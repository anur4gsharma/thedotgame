...
export function normalizeBoardConfig(input: BoardConfig): BoardConfig | undefined {
  ...
  return { width: input.width, height: input.height, visual };
}
...
export function getBoard(id: string, size?: number, config?: BoardConfig): BoardDefinition | undefined {
  if (config) {
    ...
    return generateSquareBoard(normalized.width, normalized.height, id, `${normalized.width}×${normalized.height} Custom`, normalized.visual);
  }
